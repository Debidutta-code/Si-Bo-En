...
export const getMyProfileApi = async () => {
    try {
        const response = await axiosInstace.get(`${CUSTOMER_BASE}/me`, { withCredentials: true });
...
export const getAvailableSpasApi = async (bookingCode: string) => {
    try {
        const response = await axiosInstace.get(`${SPA_BASE}/available/${bookingCode}`, { withCredentials: true });
        return response.data;
    } catch (error: any) {
        return error?.response?.data ?? { success: false, message: error?.message };
    }
...
export const markSlotAsBookedApi = async (slotId: string, data: { reservationId: string; userName: string }) => {
    try {
        const response = await axiosInstace.patch(`${SPA_BASE}/slots/slots/${slotId}/book`, data, { withCredentials: true });
        return response.data;
    } catch (error: any) {
        return error?.response?.data ?? { success: false, message: error?.message };
    }
...
export const markSlotAsAvailableApi = async (slotId: string) => {
    try {
        const response = await axiosInstace.patch(`${SPA_BASE}/slots/slots/${slotId}/available`, {}, { withCredentials: true });
        return response.data;
    } catch (error: any) {
        return error?.response?.data ?? { success: false, message: error?.message };
    }
