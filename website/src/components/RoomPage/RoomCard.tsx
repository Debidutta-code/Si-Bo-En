...
import { Users, Ruler, Eye, Wifi, Coffee, Tv, Wind, Phone, Utensils, ChevronRight, Plus, Minus, ChevronDown, ChevronUp } from 'lucide-react';
...
console.log("Room Data",bookingContext)
 ...
 useEffect(() => {
  if (expandedRatePlan && onPriceUpdate && latestPrice) {
    ...
    if (currentRatePlan) {
      ...
      // Keep finalprice as latestPrice object
      onPriceUpdate({
        room,
...
}, [selectedAddons, expandedRatePlan, latestPrice]);
...
const handleBookNowClick = async (ratePlan: any) => {
  try {
    // Step 1: Always fetch the latest price first
    const payload = {
    ...
    // Step 2: Check if property has addons
    if (propertyDetails?.addonsEnabled && addons && addons.length > 0) {
      ...
      setSelectedAddons({}); // start with empty selection
      scrollToAddons();

      // ✅ Update sidebar price
      if (onPriceUpdate) {
        const basePrice = latestPrice?.baseRatePerNight || 0;
        const totalAddonsPrice = 0; // none selected yet
        const finalprice=latestPrice

        onPriceUpdate({
          room,
          ratePlan,
          selectedAddons: [], // start empty
          basePrice,
          totalAddonsPrice,
          finalprice,
        });
    ...
    // Step 3: Else — open guest modal directly
    else {
  ...
  // Calculate total selected addons price
...
const scrollToAddons = () => {
  setTimeout(() => {
    if (addonsRef.current) {
      const yOffset = -200; // Adjust this to match your header height
      const y =
        addonsRef.current.getBoundingClientRect().top + window.scrollY + yOffset;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
        <div className="flex flex-col md:flex-row">
          <div className="md:w-2/5 lg:w-1/3">
            <img 
              src={room.images?.[0] || "https://via.placeholder.com/600x400?text=No+Image+Available"} 
              alt={room.room_name}
      ...
      <div className="space-y-3">
        {room.room_price?.map((ratePlan: any, index: number) => {
          ...
          return (
            <div 
              ...
              className={`bg-white rounded-lg shadow-md overflow-hidden border-2 transition-all ${
                isExpanded ? 'border-orange-400' : 'border-gray-200'
              }`}
            >
              {/* Rate Plan Header */}
              <div className={`p-4 md:p-5 ${isCollapsed && !isExpanded ? 'pb-2' : ''}`}>
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  {/* Left Section - Rate Plan Details */}
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Perks/Benefits - Only show when not collapsed */}
                    {!isCollapsed && (
                      <>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 sm:gap-2 sm:min-w-[180px] md:min-w-[200px]">
                    ...
                    <button
                      ...
                      disabled={loadingBookNow === `${room.id}-${ratePlan.ratePlanCode}` || isExpanded}
                      className=" bg-orange-500 hover:bg-orange-600 text-white px-4 md:px-5 py-2 rounded-lg font-semibold text-sm md:text-base transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg whitespace-nowrap"
                    >
                      {loadingBookNow === `${room.id}-${ratePlan.ratePlanCode}` ? (
                ...
                <div ref={addonsRef} className="border-t-2 border-gray-200 bg-gradient-to-b from-orange-50 to-white p-4 md:p-5">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3 ">
                    {(showAllAddons ? addons : addons.slice(0, 2)).map((addon) => {
                      // Filter availabilities for booking dates only
                                      ...
                                      <div className="flex flex-wrap items-center gap-1.5 ">
                                        ...
                                        <div className='flex items-center'>
                                          ...
                                          <button
                                            ...
                                            className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-orange-500 hover:bg-orange-600 text-white flex items-center justify-center transition-colors"
                                          >
                                            <Plus size={12} />
                                          </button>
                                        </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-2">
                    ...
                    <button
                      ...
                      className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200 border border-orange-600 shadow hover:shadow-md order-1 sm:order-2"
                    >
                      {totalAddonsCount > 0
