...
export const LoyaltyContainer = ({
  ...
  const loyaltyLogo = program.BasicLoyaltyProgram?.logo?.[0] ?? null;
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!email) { toast.error("Email is required"); setIsSubmitting(false); return; }
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/loyalty/guest/register`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email, propertyId: loyaltyProgram.propertyId, metadata: otherFields }),
        }
      );
      const data = await response.json();
          ...
          {isVerifying && (
            <div className="absolute inset-0 bg-white/90 backdrop-blur-sm flex items-center justify-center z-20 rounded-xl">
              <div className="flex items-center gap-2">
                <div className="animate-spin rounded-full h-4 w-4 border-2 border-gray-200" style={{ borderTopColor: primaryColor }} />
