...
const GuestDetails: FC<IGuestDetailsProps> = ({
  ...
  // Check all required fields are filled (first + last name)
  const allFilled = guests.every(
    (g) => g.firstName.trim().length > 0 && g.lastName.trim().length > 0
  );

  const GuestCard: FC<{ guest: IAmendGuest; index: number; displayIndex: number }> = ({
    guest,
    index,
    displayIndex,
  }) => {
    const errors = guestErrors[`guest-${index}`] || {};
