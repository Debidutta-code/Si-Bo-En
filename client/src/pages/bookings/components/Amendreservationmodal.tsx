...
const PriceSummary: FC<PriceSummaryProps> = ({
  ...
  const isPayAtHotel =
    paymentMethod === "pay_at_hotel" || paymentMethod === "payAtHotel";
  const fmt = (n: number) =>
  ...
  const TaxRows = () => {
    if (!finalPrice.tax?.length) return null;
    return (
      <>
        {finalPrice.tax.map((t, i) => (
          <div
            key={i}
            className="flex justify-between text-sm text-muted-foreground"
          >
            <span>{t.name || `Tax ${i + 1}`}</span>
            <span>
        ...
        {finalPrice.totalTax > 0 && (
          <div className="flex justify-between text-sm font-medium border-t border-border pt-2 mt-1">
            <span>Total Tax</span>
            <span>{fmt(finalPrice.totalTax)}</span>
          </div>
        )}
      </>
    );
              ...
              <>
                <div className="flex justify-between text-sm">
                  <span className="text-card-foreground">
                    Base Rate ({finalPrice.numberOfNights}{" "}
                    {finalPrice.numberOfNights === 1 ? "night" : "nights"})
                  </span>
                  <span className="font-medium">
                    {fmt(finalPrice.breakdown.totalBaseAmount)}
                  </span>
                </div>
                {finalPrice.breakdown.totalAdditionalCharges > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {fmt(finalPrice.breakdown.totalAdditionalCharges)}
                    </span>
                  </div>
                )}
  ...
  return (
    <div className="rounded-lg border border-border bg-card overflow-hidden">
      ...
      <div className="px-5 py-4 grid grid-cols-2 gap-6 border-b border-border">
        ...
        <div className="flex items-start justify-between">
          ...
          {priceFetched && (
            <button
              onClick={onToggleBreakdown}
              className="flex items-center gap-1 text-xs text-primary hover:text-primary/80 transition-colors font-medium mt-0.5"
            >
              <Info className="h-4 w-4" />
              {showBreakdown ? "Hide" : "Details"}
              {showBreakdown ? (
      ...
      {/* Breakdown panel */}
      {showBreakdown && priceFetched && (
        <div className="bg-muted/40 px-5 py-4 border-b border-border space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">
            Price Breakdown
          </p>
          {finalPrice.breakdown.totalBaseAmount > 0 && (
        ...
        {priceFetched && finalPrice.booking?.finalPayable > 0 && (
          <div className="flex justify-between items-center bg-destructive/10 rounded-md px-3 py-2.5">
            ...
            <span className="text-sm font-bold text-destructive">
              {fmt(finalPrice.booking.finalPayable)}
            </span>
          </div>
        )}
        ...
        {priceFetched && finalPrice.booking?.refundAmount > 0 && (
          <div className="flex justify-between items-center bg-green-500/10 rounded-md px-3 py-2.5">
            ...
            <span className="text-sm font-bold text-green-700 dark:text-green-400">
              {fmt(finalPrice.booking.refundAmount)}
            </span>
          </div>
        )}
...
const AmendReservationModal: FC<IAmendReservationModalProps> = ({
  ...
  const [requestedRooms, setRequestedRooms] = useState<number>(originalRooms);
  const [guestForms, setGuestForms] = useState<IAmendGuest[]>(
    normalizeGuests(reservation.guests)
  );
  const [amount, setAmount] = useState<number>(reservation.amount);
  const [finalPrice, setFinalPrice] = useState<IAmendFinalPrice>(
    emptyFinalPrice()
  );
  ...
  const [dateErrors, setDateErrors] = useState<
    Pick<IAmendValidationErrors, "checkIn" | "checkOut">
  >({});
  const [guestErrors, setGuestErrors] = useState<
    Record<string, IGuestFieldErrors>
  >({});

  const checkInIsPast = isBefore(
    startOfDay(new Date(initialCheckIn)),
    startOfDay(new Date())
  );
  ...
  const validateDates = (): boolean => {
    ...
    else if (!checkInIsPast && checkIn <= today)
      errors.checkIn = "Check-in must be after today.";
  ...
  const validateGuests = (): boolean => {
    ...
    guestForms.forEach((guest, index) => {
      ...
      if (!guest.firstName.trim()) {
        gErr.firstName = "First name is required.";
        valid = false;
      } else if (!nameRegex.test(guest.firstName)) {
        gErr.firstName = "Only letters allowed.";
        valid = false;
      }
      if (!guest.lastName.trim()) {
        gErr.lastName = "Last name is required.";
        valid = false;
      } else if (!nameRegex.test(guest.lastName)) {
        gErr.lastName = "Only letters allowed.";
        valid = false;
      }
  ...
  const handleConfirm = async () => {
    ...
    if (!priceFetched) {
      toast.error("Please check availability & price before confirming.");
      return;
    }
    if (!validateDates()) return;
    if (!validateGuests()) {
      toast.error("Please fix guest details before confirming.");
      return;
    }

    const rooms: IAmendRoom[] = [
      {
      ...
      <div className="bg-card rounded-xl w-full max-w-2xl shadow-2xl border border-border my-8">
        ...
        <div className="px-6 py-4 bg-muted/50 border-b border-border">
          <p className="font-semibold text-card-foreground text-base mb-3">
            {reservation.hotelName || "Property"}
          </p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className="text-card-foreground font-medium">
                {new Date(initialCheckIn).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                })}{" "}
                →{" "}
                {new Date(initialCheckOut).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
                })}
                ...
                <div className="space-y-1.5">
                  ...
                  {checkInIsPast && (
                    <p className="text-xs text-amber-600 flex items-center gap-1.5">
                      ...
                      Cannot change — check-in date has passed
                    </p>
                  )}
                  {dateErrors.checkIn && (
              ...
              <div className="flex items-center justify-between p-4 rounded-lg border border-border bg-muted/30">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Originally booked: {originalRooms}{" "}
                    {originalRooms === 1 ? "room" : "rooms"}
                  </p>
                </div>
        ...
        <div className="px-6 pb-6 space-y-4">
          ...
          <div className="rounded-lg bg-muted/50 border border-border px-4 py-3 space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              • Reducing length of stay may be subject to cancellation policy
            </p>
          </div>
