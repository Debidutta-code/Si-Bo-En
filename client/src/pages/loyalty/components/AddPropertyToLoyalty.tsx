...
export default function AddPropertyToLoyalty({ 
  ...
  const handleRemoveProperty = async (propertyConfigId: string): Promise<void> => {
    if (!confirm("Are you sure you want to remove this property from the loyalty program?")) return;

    setIsLoading(true);
    try {
      const response = await deletePropertyLoyalityConfigService(propertyConfigId);
      if (response.success) {
  ...
  const handleToggleStatus = async (property: IPropertyLoyaltyConfig): Promise<void> => {
    setIsLoading(true);
    try {
      const response = await updatePropertyLoyalityConfigService(property.id, !property.isActive);
      if (response.success) {
