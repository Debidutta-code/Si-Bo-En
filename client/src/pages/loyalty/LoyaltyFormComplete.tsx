...
import { getAllFieldService, getFieldsService, updateManyFieldsService, addFieldsService } from "./services";
...
export default function LoyaltyForm() {
  ...
  const fetchFields = async () => {
    ...
    try {
      ...
      if (allFieldsResponse.success && allFieldsResponse.data) {
        const masterFields = allFieldsResponse.data.map((field: any) => ({
          id: field.id || field._id,
          name: field.name || field.fieldName,
          apiCode: field.apiCode || field.masterRegistrationFieldId,
  ...
  const getAvailableFieldsToAdd = () => {
    const configuredApiCodes = new Set(configuredFields.map(f => f.apiCode));
    return availableMasterFields.filter(field => !configuredApiCodes.has(field.apiCode));
  };
    ...
    try {
      const fieldsToAdd = Array.from(selectedFields).map(fieldId => {
        ...
        return {
          ...
          masterRegistrationFieldId: masterField?.apiCode || fieldId,
      ...
      for (const field of fieldsToAdd) {
        const response = await addFieldsService(field);
        if (!response.success) {
          allSuccess = false;
          toast.error(`Failed to add ${field.fieldName}`);
        }
      }
  ...
  const handleCheckboxChange = (index: number, field: keyof IFieldConfig) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await updateManyFieldsService(creationId, fieldsToUpdate);
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 lg:p-8">
      <div className="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>Loyalty</span>
        <span>/</span>
        <span className="text-foreground font-medium">Registration Form</span>
      </div>
        ...
        <CardContent>
            ...
            <table className="w-full">
              <thead className="border-b">
                <tr className="text-left">
                  <th className="pb-3 pr-4 font-medium text-sm"></th>
                  <th className="pb-3 pr-4 font-medium text-sm">Field</th>
                  <th className="pb-3 pr-4 font-medium text-sm">API Code</th>
                  ...
                  <th className="pb-3 pl-4 font-medium text-sm">Order</th>
                </tr>
              </thead>
              <tbody>
          ...
          {configuredFields.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              No fields configured yet. Click "Add Fields" to get started.
            </div>
          )}
        </CardContent>
      </Card>
