  ...
  useEffect(() => {
    if (editData) {
      ...
      if (editData.roomRatePlans && editData.roomRatePlans.length > 0) {
        ...
        const ratePlanIds = editData.roomRatePlans
          .map(rp => rp.ratePlanId)
          .filter((id): id is string => !!id);
        setSelectedRooms([...new Set(roomIds)]);
        setSelectedRatePlans([...new Set(ratePlanIds)]);
      }
    }
  }, [editData]);
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          {(selectionMode === 'specific' || editData) && (
            <div className="space-y-4 p-4 bg-muted/20 rounded-lg border border-border">
              ...
              <div className="space-y-3">
                ...
                {editData ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {editData.roomRatePlans?.map((rp, idx) => (
                      <div key={idx} className="px-3 py-2 bg-muted/30 border border-border rounded-md">
                        <div className="text-sm font-medium text-foreground truncate">
                          {rp.roomType || 'Room Type'}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="space-y-3">
                ...
                {editData ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {[...new Set(editData.roomRatePlans?.map(rp => rp.ratePlanCode))].map((code, idx) => {
                      return (
                        <div key={idx} className="px-3 py-2 bg-muted/30 border border-border rounded-md">
                          <div className="text-sm font-medium text-foreground truncate">
                            {editData.ratePlan?.ratePlanName || 'Rate Plan'}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            ({code})
                          </div>
                ...
                <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                  <p className="text-xs text-blue-700 dark:text-blue-300">
                    {selectedRooms.length === 0 && selectedRatePlans.length > 0 && (
                      <>Selected rate plans will apply to <span className="font-semibold">all room types</span></>
                    )}
                    {selectedRooms.length > 0 && selectedRatePlans.length > 0 && (
                      <>
                        Promotion will apply to <span className="font-semibold">{selectedRooms.length} room type(s)</span> with{' '}
                        <span className="font-semibold">{selectedRatePlans.length} rate plan(s)</span>
                      </>
                    )}
                    {selectedRatePlans.length === 0 && (
                      <span className="text-destructive font-medium">Please select at least one rate plan</span>
                    )}
                  </p>
                </div>
              )}
          ...
          {selectionMode === 'all' && !editData && (
            <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
              <p className="text-xs text-blue-700 dark:text-blue-300">
                This promotion will be applied to <span className="font-semibold">all {roomTypes.length} room type(s)</span> with{' '}
                <span className="font-semibold">all {ratePlans.length} rate plan(s)</span>
              </p>
            </div>
          )}
        </div>
