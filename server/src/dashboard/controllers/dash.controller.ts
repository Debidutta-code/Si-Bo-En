...
export class DashBoardController {
    ...
    public async getAnalytics(req: CustomRequest, res: Response): Promise<Response> {
        try {
            ...
            const serRes = await this.dashboardServices.getPropertyIdsAndCodesServices(
                req.user.creationId,
                req.user.level,
                propertyId?.toString(),
    ...
    public async getStatisticsComparison(req: CustomRequest, res: Response): Promise<Response> {
        try {
            ...
            const { comparisonType = 'date', selectedDate = new Date().toISOString(), propertyId, propertyCode, propertyName, selectedCurrency } = req.query;
