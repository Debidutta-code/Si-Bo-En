...
export class SpaService {
    ...
    public async createSpa(data: ICSpaR): Promise<IApiResponse> {
        try {
            const [isExistByName,isExistByCode,{convert, baseCurrency}]=await Promise.all([
                this.spaRepository.getByName(data.name,data.propertyId),
                this.spaRepository.getSpaByCode(data.itemCode,data.propertyId),
                getCurrencyConverter(data.propertyId, data.currencyCode ? data.currencyCode : "AED")
            ]);

            if(isExistByName || isExistByCode){
                return errorResponse("Spa with the same name or code already exists");
            }
            const discountedValue=convert(data.discountValue ? data.discountValue : 0)
    ...
    public async updateSpa(id: string, data: IUSpaR): Promise<IApiResponse> {
        try {
            ...
            const [isExistByName,isExistByCode,{convert, baseCurrency}]=await Promise.all([
                ...
                this.spaRepository.getSpaByCode(data.itemCode,isExist.propertyId),
                getCurrencyConverter(isExist.propertyId, data.currencyCode ? data.currencyCode : "AED")
            ]);
            ...
            await this.spaRepository.updateSpa(id, {
                ...data,
                currencyCode:data.currencyCode?baseCurrency:null,
                discountValue:data.discountValue?convert(data.discountValue):null
            });
