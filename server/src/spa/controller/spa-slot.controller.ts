...
export class SpaDateController {
    ...
    public async createSpaDate(req: CustomRequest, res: Response): Promise<Response<IApiResponse>> {
        try {
            const spaId = req.params.id;
            const data = req.body;
            if(!data||!data.date){
                return res.status(400).json(errorResponse("Date is required for creating the spa date"))
            }
            const date = toUTC(data.date);
            const response = await this.spaDateService.createSpaDate(date, spaId);
            return res.status(response.success? 200 : 400).json(response);
        } catch (error) {
            if(error instanceof Error){
                return res.status(500).json(errorResponse("Failed to create spa date", error.message));
            }
            return res.status(500).json(errorResponse("Failed to create spa date", "Unknown error")); 
        }
    }
    public async getSpaForDateRange(req: CustomRequest, res: Response): Promise<Response<IApiResponse>> {
        try {
            const spaId = req.params.id;
            ...
            const response = await this.spaDateService.getSpaForDateRange(spaId, toUTC(startDate), toUTC(endDate));
            return res.status(response.success ? 200 : 400).json(response);
        } catch (error) {
            if (error instanceof Error) {
                return res.status(500).json(errorResponse("Failed to fetch spa dates for range", error.message));
            }
            return res.status(500).json(errorResponse("Failed to fetch spa dates for range", "Unknown error"));
        }
    }
    public async deleteSpaDate(req: CustomRequest, res: Response): Promise<Response<IApiResponse>> {
        try {
            const spaId = req.params.id;
            if(!spaId){
                return res.status(400).json(errorResponse("Spa ID is required"));
            }
            const response = await this.spaDateService.deleteDate(spaId);
            return res.status(response.success ? 200 : 400).json(response);
        } catch (error) {
            if (error instanceof Error) {
                return res.status(500).json(errorResponse("Failed to delete spa date", error.message));
            }
            return res.status(500).json(errorResponse("Failed to delete spa date", "Unknown error"));
        }
    }
}
...
export class SpaSlotController {
    ...
    public async createSlots(req: CustomRequest, res: Response): Promise<Response<IApiResponse>> {
        try {
            ...
            return res.status(500).json(errorResponse("Failed to create spa slots", "Unknown error"));
        }
    }
    public async deleteSpaSlot(req: CustomRequest, res: Response): Promise<Response<IApiResponse>> {
        try {
            ...
            if (error instanceof Error) {
                return res.status(500).json(errorResponse("Failed to delete spa slot", error.message));
            }
    ...
    public async markSlotAsBooked(req: CustomRequest, res: Response): Promise<Response<IApiResponse>> {
        try {
            ...
            if (error instanceof Error) {
                return res.status(500).json(errorResponse("Failed to mark spa slot as booked", error.message));
            }
            return res.status(500).json(errorResponse("Failed to mark spa slot as booked", "Unknown error"));
        }
    }
    public async markSlotAsAvailable(req: CustomRequest, res: Response): Promise<Response<IApiResponse>> {
        try {
            const slotId = req.params.id;
            if (!slotId) {
                return res.status(400).json(errorResponse("Slot ID is required"));
