...
export class GroupSearchController {
  ...
  public async getPropertiesByGroup(
    ...
    try {
      ...
      if (!groupId) {
        return res.status(400).json(errorResponse('groupId is required'));
      }

      const body: IGroupSearchQuery = req.body;
      if (!body.startDate || !body.endDate || !body.guests) {
        return res.status(400).json(
          errorResponse('startDate, endDate and guests are required')
        );
      }

      const result = await this.groupSearchService.getPropertiesByGroup(groupId, body);
...
public async getBrandsByGroup(
  ...
  try {
    ...
    const result = await this.groupSearchService.getBrandsByGroup(groupId);
    return res.status(result.success ? 200 : 400).json(result);
  } catch (error) {
    return res.status(500).json(
      errorResponse(
  ...
  public async getPropertiesByBrand(
    ...
    try {
      ...
      if (!brandId) {
        return res.status(400).json(errorResponse('brandId is required'));
      }
      ...
      const result = await this.groupSearchService.getPropertiesByBrand(brandId, body);
      return res.status(result.success ? 200 : 400).json(result);
    } catch (error) {
      return res.status(500).json(
        errorResponse(
