...
export class GroupSearchService {
  ...
  private async getBasePrice(
    ...
    return { basePrice: lowest.totalAmount, currencyCode: lowest.currencyCode };
  }
  ...
  private async filterAvailableProperties(
    ...
    const results = await Promise.all(
      properties.map(async (property) => {
        const priceData = await this.getBasePrice(property.propertyCode, query);
  ...
  private async resolveGroupPropertyCreationIds(groupId: string): Promise<string[]> {
    const group = await this.groupSearchRepository.getGroupChildrens(groupId);
    if (!group) return [];
    ...
    if (brandIds.length > 0) {
      const brands = await this.groupSearchRepository.getBrandChildrens(brandIds);
      brandPropertyIds = brands.flatMap((b) =>
        b.brandChildren.filter((c) => c.type === 'property').map((c) => c.id)
      );
    }
  ...
  public async getPropertiesByGroup(
    ...
    try {
      const creationIds = await this.resolveGroupPropertyCreationIds(groupId);

      if (creationIds.length === 0) {
        return successResponse('No properties found for this group', []);
      }

      const properties = await this.groupSearchRepository.getPropertyDetails(
        creationIds,
        query.city
      );
      ...
      const available = await this.filterAvailableProperties(properties, query);
      ...
      return successResponse('Properties retrieved successfully', available);
    } catch (error) {
      return error instanceof Error
        ? errorResponse('Failed to retrieve properties', error.message)
        : errorResponse('Failed to retrieve properties', 'Unknown error');
    }
  }
  ...
  public async getBrandsByGroup(
    ...
    try {
      ...
      const brandNames = await this.groupSearchRepository.getBrandNames(brandIds);
  ...
  public async getPropertiesByBrand(
    ...
    try {
      const creationIds =
        await this.groupSearchRepository.getPropertyCreationIdsByBrand(brandId);

      if (creationIds.length === 0) {
        return successResponse('No properties found for this brand', []);
      }
