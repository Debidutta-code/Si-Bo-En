...
export class ReservationService {
    ...
    public async getReservationById(
        ...
        try {
            ...
            return successResponse('Reservation fetched successfully', {
                reservation,
            });
        } catch (error) {
            if (error instanceof Error) {
    ...
    public async getReservationByBookingCode(
        ...
        try {
            ...
            if (!reservation) {
                return errorResponse(
                    'Reservation not found',
                    'No reservation found with the provided booking code'
                );
            }
    ...
    public async cancelReservation(
        ...
        try {
            ...
            const checkInDate = new Date(
                existingReservation.reservationStartDate
            );
    ...
    public async getReservationStats(agencyId: string): Promise<IApiResponse> {
        try {
            const stats =
                await this.reservationRepository.getReservationStats(agencyId);
            ...
            return successResponse(
                'Reservation statistics fetched successfully',
                {
                    stats: {
                        ...stats,
                        averageBookingValue,
    ...
    public async getUpcomingArrivals(
        ...
        try {
            const arrivals =
                await this.reservationRepository.getUpcomingArrivals(
                    agencyId,
                    days
                );
            ...
            if (error instanceof Error) {
                return errorResponse(
                    'Failed to fetch upcoming arrivals',
                    error.message
                );
            }
            return errorResponse(
    ...
    public async getUpcomingDepartures(
        ...
        try {
            const departures =
                await this.reservationRepository.getUpcomingDepartures(
                    agencyId,
                    days
                );
