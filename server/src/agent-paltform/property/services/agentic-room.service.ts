...
export class AgenticRoomService {
    ...
    public async getRoomDetails(
        ...
        try {
            ...
            if (!raw.propertyConfigs?.isB2bAvailable) {
                return errorResponse('Property not available', 'This property is not enabled for B2B bookings');
            }
    ...
    private async processRoom(
        ...
        const amenities: IRoomAmenity[] = room.roomAmenities.map((r: IRoomAmenity) => ({
            amenity: {
    ...
    private async processRatePlan(
        ...
        const [charges, ratePlanRule, touristTaxData, bookingOffset, ratePlanAddons] =
            await Promise.all([
        ...
        const effectiveRoomsArray = roomsArray.length > 0
            ? roomsArray
            : [{ adults: guests.adults, children: guests.children, childAges: [] }];
        ...
        const { calculatedCommissionAmount, appliedCommission } =
            await this.calculateCommission(baseAmount, agencyCommission, property.id, property.propertyConfigs);
    ...
    private validateRestrictions(
        ...
        if (ratePlanRule?.isActive) {
            ...
            if (withinPeriod) {
                if (ratePlanRule.minLos !== null && numberOfNights < ratePlanRule.minLos) return false;
                if (ratePlanRule.maxLos !== null && numberOfNights > ratePlanRule.maxLos) return false;
            }
        }
    ...
    private validateOccupancy(
        ...
        const gap = Math.max(
            0,
            room.maxOccupancy - room.maxNumberOfAdults - room.maxNumberOfChildren
        );
    ...
    private calculateBasePrice(
        ...
        const adultBaseAmounts = charge.baseGuestAmounts
            ...
            .sort((a: IBaseByGuest, b: IBaseByGuest) => a.numberOfGuests - b.numberOfGuests);
        ...
        const additionalAdultCharge = charge.additionalGuestAmounts
            .find((a: IChargeAdditionalGuest) => a.ageQualifyingCode === '10');

        const additionalChildCharge = charge.additionalGuestAmounts
            .find((a: IChargeAdditionalGuest) => a.ageQualifyingCode === '8');
        ...
        const childResult: IGuestPriceResult = guests.children > 0
            ? this.calculateGuestTypePrice(guests.children, childBaseAmounts, additionalChildCharge) ?? { basePrice: 0, additionalCharges: 0 }
            : { basePrice: 0, additionalCharges: 0 };

        const baseAmount =
            adultResult.basePrice + adultResult.additionalCharges +
            childResult.basePrice + childResult.additionalCharges;
    ...
    private calculateGuestTypePrice(
        ...
        const exactBase = baseAmounts.find((b: IBaseByGuest) => b.numberOfGuests === guestCount);
        if (exactBase) {
            return { basePrice: Number(exactBase.amountBeforeTax), additionalCharges: 0 };
        }
        ...
        if (baseAmounts.length > 0) {
            ...
            if (extraGuests > 0) {
                ...
                return { basePrice, additionalCharges: extraGuests * Number(additionalCharge.amount) };
            }
            return { basePrice, additionalCharges: 0 };
        }

        if (additionalCharge) {
            return { basePrice: 0, additionalCharges: guestCount * Number(additionalCharge.amount) };
        }
    ...
    ): Promise<{ calculatedCommissionAmount: number; appliedCommission: IAppliedCommission }> {
        let calculatedCommissionAmount = 0;

        if (commission.commissionType === 'percentage') {
            calculatedCommissionAmount = Number((baseAmount * (commission.commissionValue / 100)).toFixed(2));
            ...
            if (!commissionCurrency || commissionCurrency === propertyCurrency) {
                calculatedCommissionAmount = Number(commission.commissionValue.toFixed(2));
            } else {
                const converter = await getCurrencyConverter(propertyId, commissionCurrency);
                calculatedCommissionAmount = Number(converter.convert(commission.commissionValue).toFixed(2));
            }
        }
    ...
    private calculateTouristTax(
        ...
        console.log("data", touristTaxData, baseAmount, numberOfNights, numberOfRooms, numberOfBedrooms)
        const calculatedTaxAmount =
            touristTaxData.discountType === 'percentage'
                ? Number((baseAmount * (Number(touristTaxData.discountValue) / 100)).toFixed(2))
                : Number((Number(touristTaxData.discountValue) * numberOfNights * numberOfRooms * numberOfBedrooms).toFixed(2));
...
class AgenticAddonCalculator {
    ...
    private calculateAddonPrice(
        ...
        const getRoomGuestPrice = (room: IRoomConfig): number =>
            ...
            room.childAges.reduce((sum: number, age: number) => sum + getChildPrice(age), 0);
        ...
        switch (addon.postingRhythm) {
            ...
            case 'per_person_per_stay':
                return this.roomsArray.reduce(
                    (sum: number, room: IRoomConfig) => sum + getRoomGuestPrice(room), 0
                );
            ...
            case 'per_person_per_room':
                return this.roomsArray.reduce(
                    (sum: number, room: IRoomConfig) => sum + getRoomGuestPrice(room) * this.numberOfRooms, 0
                );
            default:
                return 0;
        }
