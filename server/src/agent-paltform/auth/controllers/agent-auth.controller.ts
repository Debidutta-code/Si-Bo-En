...
export class AgentAuthController {
    ...
    public async getMe(req: AgentRequest, res: Response): Promise<Response> {
        try {
            ...
            if (!userId) {
                return res.status(401).json(errorResponse("UnAuthorized User, Log in again ", "No Id found in request"));
            }
            const agent = await this.authService.getMeAgent(userId)
            return res.status(agent ? 200 : 401).json(agent);
        } catch (error) {
            if (error instanceof Error) {
                return res.status(500).json(errorResponse("Failed to retrieve agent", error.message));
            }
            return res.status(500).json(errorResponse("Internal Server Error"));
        }
    }
    public async logout(req: AgentRequest, res: Response): Promise<Response> {
        try {
            ...
            if (error instanceof Error) {
                return res.status(500).json(errorResponse("Logout failed", error.message));
            }
            return res.status(500).json(errorResponse("Internal Server Error"));
        }
    }
