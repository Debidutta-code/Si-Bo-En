...
export class AgentAuthService {
    ...
    public async login(agentLogin: IAgentLogin): Promise<IApiResponse> {
        try {
            const agent = await this.agentAuthRepository.findAgentByEmail(agentLogin.email);
            if (!agent) {
            ...
            return successResponse("Agent Login successful", { agent, accessToken });
        } catch (error) {
            if(error instanceof Error){
                return errorResponse("Login failed", error.message);
            }
            return errorResponse("Login failed", "An unexpected error occurred during login");
        }
    }
    public async getMeAgent(id:string): Promise<IApiResponse> {
        try {
            ...
            return errorResponse("Failed to retrieve agent", "An unexpected error occurred");
        }
    }
}