...
export class AgencyService {
    ...
    public async getAgencies(page: number, limit: number): Promise<IApiResponse> {
        try {
            ...
            if (error instanceof Error) {
                return errorResponse("failed to retrieve agencies", error.message);
            }
    ...
    public async createAgency(data: ICAgency): Promise<IApiResponse> {
        try {
            const isAlreadyExists = await this.agencyRepository.getAgencyByAgencyCreds(data.agencyEmail, data.taxNo, data.agencyName);
            if (isAlreadyExists) {
                return errorResponse("Agency with the same credentials already exists","agency exists with same email, or tax or name");
            }
    ...
    public async updateAgency(agencyId: string, data: ICAgency): Promise<IApiResponse> {
        try {
            const existingAgency = await this.agencyRepository.getAgencyById(agencyId);
            if (!existingAgency) {
            ...
            const agency = await this.agencyRepository.updateAgency(agencyId, data);
    ...
    public async getReservationsForAgency(agencyId: string, page: number=1, limit: number=10): Promise<IApiResponse> {
        try {
            const reservations = await this.agencyRepository.getReservationsByAgencyId(agencyId, (page - 1) * limit, limit);
            return successResponse("Reservations retrieved successfully", reservations);
        } catch (error) {
            if (error instanceof Error) {
