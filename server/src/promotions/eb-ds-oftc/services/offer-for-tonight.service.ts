...
export class OfferForTonightPromotionService {
  ...
  public  async getOfferForTonightPromotionsByProperty(propertyId: string): Promise<IApiResponse> {
    try {
      const promotions = await this.offerForTonightPromotionDao.getOfferForTonightPromotionsByProperty(propertyId);
  ...
  public  async getOfferForTonightPromotionById(id: string): Promise<IApiResponse>  {
    try {
      const promotion = await this.offerForTonightPromotionDao.getOfferForTonightPromotionById(id);
  ...
  public  async updateOfferForTonightPromotion(id: string, updateData: ICEbDsOftc): Promise<IApiResponse>  {
    try {
      const existingPromotion = await this.offerForTonightPromotionDao.getOfferForTonightPromotionById(id);
  ...
  public  async deleteOfferForTonightPromotion(id: string): Promise<IApiResponse>  {
    try {
      const existingPromotion = await this.offerForTonightPromotionDao.getOfferForTonightPromotionById(id);
      if (!existingPromotion) {
        return errorResponse('Offer-for-tonight promotion not found');
      ...
      const deletedPromotion = await this.offerForTonightPromotionDao.deleteOfferForTonightPromotion(id);
