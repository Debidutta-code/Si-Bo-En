...
import { 
  ...
  IOfferForTonightPromotionUpdate,
  IRoomRatePlanPair,
...
export class OfferForTonightPromotionDao {
  /**
   * Create offer-for-tonight promotions (one per room-rateplan pair)
   */
  public static async createOfferForTonightPromotions(
    ...
    try {
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to create offer-for-tonight promotions: ${error.message}`);
      }
   ...
   */
  public static async getOfferForTonightPromotionsByProperty(
    propertyId: string
  ): Promise<any[]> {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to fetch offer-for-tonight promotions: ${error.message}`);
      }
   ...
   */
  public static async getOfferForTonightPromotionById(id: string): Promise<any | null> {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to fetch offer-for-tonight promotion: ${error.message}`);
      }
   ...
   */
  public static async updateOfferForTonightPromotion(
    id: string,
    updateData: IOfferForTonightPromotionUpdate
  ): Promise<any> {
    try {
      const data: any = {};
...
data.isAutoApplied = updateData.isAutoApplied;
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to update offer-for-tonight promotion: ${error.message}`);
      }
   ...
   */
  public static async deleteOfferForTonightPromotion(id: string): Promise<any> {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to delete offer-for-tonight promotion: ${error.message}`);
      }
