...
export class RestrictionRepository {
  public static async applyRestrictions(
    ...
    try {
      const restrictionField = restrictionType === 'CTA' 
        ? 'isClosedToArrival' 
        : 'isClosedToDeparture';
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to apply restrictions: ${error.message}`);
      }
      throw new Error('Unknown error occurred while applying restrictions');
    }
  }
  ...
  public static async getRestrictionsByProperty(
    ...
    try {
      ...
      throw new Error('Unknown error occurred while fetching restrictions');
    }
  }
}