...
import type { CreatePolicyData, IPolicy, PolicyFilters, UpdatePolicyData, FilterOptions, PaginatedResult, allPolicies } from "../types/type"
...
export class PolicyRepository {
  ...
  public static async findOne({
    ...
    try {
      ...
      const policyId = ratePlan.depositPolicyId || ratePlan.cancellationPolicyId || ratePlan.guaranteePolicyId;
  ...
  public static async MapPolicyDao(
    id: string,
    ...
    try {
        ...
        for (const ratePlan of ratePlans) {
          ...
          if (ratePlan.guaranteePolicyId) {
            const guaranteePolicy = await prisma.policy.findUnique({
              where: { id: toStringId(ratePlan.guaranteePolicyId) },
            });
            if (guaranteePolicy) {
          ...
          if (ratePlan.cancellationPolicyId) {
            const cancellationPolicy = await prisma.policy.findUnique({
              where: { id: toStringId(ratePlan.cancellationPolicyId) },
            });
      ...
      const nonMappedPolicies = await prisma.policy.findMany({
        where: {
          propertyId: propertyId,
          id: {
            notIn: mappedPolicyIds.length > 0 ? mappedPolicyIds : undefined,
          },
        },
      });
      ...
      throw new Error(`Failed to get policies with rate plans: ${error.message}`);
    }
  }
  ...
  public static async AddPolicyToRatePlan(policyId: string, ratePlanId: string, policyType: "deposit" | "guarantee" | "cancellation") {
    try {
      const ratePlan = await prisma.ratePlan.findUnique({
      ...
      throw new Error(`Failed to add policy to rate plan: ${error.message}`);
    }
  }
}