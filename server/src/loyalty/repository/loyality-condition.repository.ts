...
export class LoyalityConditionRepository {
    public async createLoyalityCondition(data:ICLoyalityCondition):Promise<ILoyalityCondition> {
        try {
    ...
    public async deleteLoyalityCondition(id:string):Promise<ILoyalityCondition> {
        try {
            return await prisma.loyaltyConditions.update({
                where: { id },
                data: { isDeleted: true }
...
export class LoyalitySpecialConditionRepository {
    public async createLoyalitySpecialCondition(data:ICLoyalitySpecialCondition):Promise<ILoyalitySpecialCondition> {
        try {
    ...
    public async deleteLoyalitySpecialCondition(id:string):Promise<ILoyalitySpecialCondition> {
        try {
            return await prisma.loyaltySpecialCondition.update({
                where: { id },
                data: { isDeleted: true }
    ...
    public async getSpecialConditionsByProgramId(loyaltyProgramId:string):Promise<ILoyalitySpecialCondition[]> {
        try {
            return await prisma.loyaltySpecialCondition.findMany({
                where: { loyaltyProgramId ,isDeleted:false}
            });
        } catch (error) {
            throw new Error("Error fetching loyalty special conditions by program id " );
        }
    }
}