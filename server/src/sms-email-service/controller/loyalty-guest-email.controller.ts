...
export const sendOtp = async (
    ...
    try {
        ...
        if (!result.success) {
            const status =
                result.message === 'Loyalty guest not found' ? 404 : 429;
            return res
                .status(status)
...
export const sendPasswordResetLink = async (
    ...
    try {
        const result =
            await loyaltyGuestEmailService.sendPasswordResetLink(email);

        if (!result.success) {
            const status =
                result.message === 'Loyalty guest not found' ? 404 : 500;
            return res
                .status(status)
                .json({ success: false, message: result.message });
        }
