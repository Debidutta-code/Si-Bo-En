...
public async getByName(name:string,type:platformType):Promise<IMasterIntegrations|null>{
  try {
...
export class IMasterIntegrationFieldsRepository {
  public async createMasterIntegrationFields(data: ICMasterIntegrationIntegrationFields[], masterIntegrationId: string): Promise<IMasterIntegrationFields[]> {
    try {
      ...
      return await prisma.$transaction(async (tx) => {
        ...
        for (const field of data) {
          ...
          const exists = await tx.masterIntegrationRequiredFields.findFirst({
            where: {
              name: field.name,
              masterIntegrationId
            }
          ...
          if (!exists) {
            const created = await tx.masterIntegrationRequiredFields.create({
              data: {
                name: field.name,
                masterIntegrationId
              }
      ...
      throw new Error("Failed to add required fields to master integration")
    }
  }
  ...
  public async getFieldByName(name: string, masterIntegrationId: string): Promise<ICMasterIntegrationIntegrationFields | null> {
    try {
...
export class IMasterIntegrationUrlFieldRepository {
  public async createMasterIntegrationUrlFields(data: ICMasterIntegrationUrlFields[], masterIntegrationId: string): Promise<IMasterIntegrationUrlFields[]> {
    try {
      return await prisma.$transaction(async (tx) => {
        const createdFields: IMasterIntegrationFields[] = [];
        ...
        for (const field of data) {
          // Check if field already exists
          const exists = await tx.masterIntegrationURLFields.findFirst({
            where: {
              name: field.name,
              url: field.url,
              masterIntegrationId
          ...
          if (!exists) {
            const created = await tx.masterIntegrationURLFields.create({
              data: {
                name: field.name,
                url: field.url,
  ...
  public async getFieldByName(name: string, masterIntegrationId: string): Promise<IMasterIntegrationUrlFields | null> {
    try {
      return await prisma.masterIntegrationRequiredFields.findUnique({
        where: {
          masterIntegrationId_name: {
