...
export class RoomWishService {
    ...
    public async addRoomToWishlist(roomId: string, userId: string): Promise<IApiResponse> {
        try {
            ...
            const [propertyWishList,propertyDetails] = await Promise.all([
                this.propertyWishList.checkIfPropertyWishListExists(userId,roomDetails.propertyId),
            ...
            if (!propertyWishList) {
                const newPropertyWishList = await this.propertyWishList.createPropertyWishList({
                    otaGuestId: userId,
    ...
    public async removeRoomFromWishlist(roomId: string, userId: string): Promise<IApiResponse> {
        try {
            const roomDetails = await this.roomWishList.getRoomById(roomId);
            ...
            const propertyWishlist = await this.propertyWishList.checkIfPropertyWishListExists(userId, roomDetails.propertyId);
            if (!propertyWishlist) {
                return errorResponse("Property not found in wishlist");
            }
            const alreadyAdded = await this.roomWishList.checkIfRoomExistsInWishlist(propertyWishlist.id, roomId);
            if (alreadyAdded) {
                return errorResponse("Room already added to wishlist");
            }
            const removedItem = await this.roomWishList.removeRoomFromWishList(propertyWishlist.id, roomId);
            ...
            if(error instanceof Error){
                return errorResponse("Error removing room from wishlist",error.message);
            }
            return errorResponse("Error removing room from wishlist");
        }
    }
    public async getRoomsInWishlist(roomId: string): Promise<IApiResponse> {
        try {
            ...
            if (error instanceof Error) {
                return errorResponse("Error fetching rooms from wishlist", error.message);
            }
