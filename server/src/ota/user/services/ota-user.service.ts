...
export class OtaUserService {
    ...
    public async registerUser(ICUser: ICUser): Promise<IApiResponse> {
        try {
            const [existingUserforOTA, existAsLoyalityUser, passwordHash] =
                await Promise.all([
            ...
            if (existingUserforOTA) {
                return errorResponse(
                    'An OTA user with this email already exists',
                    'User already exists'
                );
            }
            const createdUser = await this.otaUserRepository.createUser({
                ...ICUser,
    ...
    public async updateUserPassword(
        ...
        try {
            ...
            const updatedUser = await this.otaUserRepository.updateUserPassword(
                userId,
                passwordHash
            );
            if (!updatedUser) {
    ...
    public async updateUserDetails(
        ...
        try {
            ...
            const updatedUser = await this.otaUserRepository.updateUserProfile(
                userId,
                userDetails
            );
            if (!updatedUser) {
