...
export class Users {
    ...
    public static async getUsersForProperty(
        creationId: string
    ): Promise<IRUsers[]> {
        try {
            return await prisma.user.findMany({
...
export class UtilsRepository {
    ...
    public static async groupBrands(
        groupId: string
    ): Promise<IGroupEntity | null> {
        try {
    ...
    public static async getBrandProperties(
        brandId: string
    ): Promise<IBrandEntity | null> {
        try {
            return await prisma.creation.findUnique({
    ...
    public static async getUserForCreations(
        creationIds: string[]
    ): Promise<IRUsers[]> {
        try {
            return await prisma.user.findMany({
            ...
            throw new Error(
                `Failed to get users for creations: ${error.message}`
            );
        }
    }
}
