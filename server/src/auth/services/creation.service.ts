...
export default class CreationService {
    public static async create(
        ...
        try {
            ...
            if (isCustom) {
                customCreation =
                    await CreationRepository.getSpecificCreation(assignTo);
            }
            if (isCustom && !customCreation) {
                return errorResponse('Custom creation not found');
            }
            if (isCustom && customCreation) {
                superCreation = await CreationRepository.getSpecificCreation(
                    customCreation.superId!
                );
            }
            ...
            switch (userLevel) {
                case 2:
                    daoRes = await CreationRepository.create(
                        type,
                        name,
                        userId,
                        undefined,
                ...
                case 3:
                    daoRes = await CreationRepository.create(
                        ...
                        userId,
                        isCustom
                            ? (customCreation?.superId ?? undefined)
                            : undefined,
    ...
    public static async update(
        ...
        try {
                    ...
                    await Promise.all(
                        imagesToDelete.map(img =>
                            deleteFileByUrl(img).catch(err =>
                                console.error(
                                    'Failed to delete creation image:',
                                    img,
                                    err
                                )
                            )
            ...
            const daoRes = await CreationRepository.update(
                creationId,
                name,
                images,
                isActive
    ...
    public static async addUser(
        ...
        try {
            let daoRes: any;
            if (userLevel === 0) {
                daoRes = await ManageCreationUser.addLevel0User(
                    userId,
                    creationId
                );
            } else if (userLevel == 1) {
                daoRes = await ManageCreationUser.addLevel1User(
                    userId,
                    creationId
                );
            } else if (userLevel == 2) {
                daoRes = await ManageCreationUser.addLevel2User(
                    userId,
                    creationId
                );
            } else if (userLevel == 3) {
                daoRes = await ManageCreationUser.addLevel3User(
                    userId,
                    creationId
                );
            } else if (userLevel == 4) {
                daoRes = await ManageCreationUser.addLevel4User(
                    userId,
                    creationId
                );
            } else {
    ...
    public static async removeUser(
        ...
        try {
            let daoRes: any;
            if (userLevel === 0) {
                daoRes = await ManageCreationUser.removeLevel0User(
                    userId,
                    creationId
                );
            } else if (userLevel == 1) {
                daoRes = await ManageCreationUser.removeLevel1User(
                    userId,
                    creationId
                );
            } else if (userLevel == 2) {
                daoRes = await ManageCreationUser.removeLevel2User(
                    userId,
                    creationId
                );
            } else if (userLevel == 3) {
                daoRes = await ManageCreationUser.removeLevel3User(
                    userId,
                    creationId
                );
            } else {
    ...
    public static async toggleDraft(creationId: string, val: boolean = true) {
        try {
            const daoRes = await CreationRepository.toggleDraft(
                creationId,
                val
            );
            if (daoRes) {
            ...
            if (!creation) {
                ...
                if (creation.type === 'property') {
                    if (!creation.propertyId) {
                        return successResponse(
                            'Creation found But property has not created',
                            { creation, propertyDetails: null }
                        );
                    }
                    let propertyDetails =
                        await PropertyDao.getPropertyByIdForCreation(
                            creation.propertyId
                        );
                    return successResponse(
                        'Creation found with property details',
                        { creation, propertyDetails: propertyDetails }
                    );
                }
                ...
                if (creationAny.groupChildren?.length) {
                    ...
                    for (const child of creationAny.groupChildren) {
                        if (
                            child.type === 'brand' &&
                            child.brandChildren?.length
                        ) {
                            const { brandChildren, ...brandWithoutChildren } =
                                child;
    ...
    public static async addCreationToCreation(
        parentCreationId: string,
        creationIdToBeAdded: string
    ) {
        {
            try {
                const parentCreationDetails =
                    await CreationRepository.getSpecificCreation(
                        parentCreationId
                    );
                ...
                switch (parentCreationDetails.type) {
                    case 'super':
                        return await AddCreationToCreation.addToSuper(
                            parentCreationId,
                            creationIdToBeAdded
                        );

                    case 'group':
                        return await AddCreationToCreation.addToGroup(
                            parentCreationId,
                            creationIdToBeAdded
                        );

                    case 'brand':
                        return await AddCreationToCreation.addToBrand(
                            parentCreationId,
                            creationIdToBeAdded
                        );
                    case 'regional':
                    ...
                    default:
                        throw new Error(
                            'Property can be only created by Brand/Group/Super'
                        );
                }
            } catch (error: any) {
...
export class FetchByCreationId {
    public static async getBrandManagers(
        ...
        try {
            switch (type) {
                case 'group':
                    const daoRes =
                        await CreationDetailsByCreationId.getGroupManagersGroup(
                            groupId
                        );
                    if (daoRes) {
                        return successResponse(
                ...
                case 'brand':
                    const gbDaoRes =
                        await CreationDetailsByCreationId.getGroupManagersBrands(
                            groupId
                        );
                    if (gbDaoRes) {
                        return successResponse(
                ...
                default:
                    return errorResponse(
                        'Invalid Type .type can only be group|brand|property'
                    );
            }
        } catch (error: any) {
            return errorResponse(
                `Failed to create groupManagers ${type}`,
                error?.message
            );
        }
    }
    public static async getGroupManagers(
        brandId: string,
        type: 'brand' | 'property'
    ) {
        try {
            switch (type) {
                case 'brand':
                    const bbDaoRes =
                        await CreationDetailsByCreationId.getBrandManagersBrand(
                            brandId
                        );
                    if (bbDaoRes) {
                        return successResponse(
                ...
                case 'property':
                    const bpDaoRes =
                        await CreationDetailsByCreationId.getBrandManagersProperty(
                            brandId
                        );
                    if (bpDaoRes) {
                        return successResponse(
                ...
                default:
                    return errorResponse(
                        'Invalid Type!. type can only be brand|property'
                    );
            }
        } catch (error: any) {
            return errorResponse(
    ...
    public static async getPropertyManagers(
        propertyId: string,
        type: 'property'
    ) {
        try {
            const ppDaoRes =
                await CreationDetailsByCreationId.getPropertyManagersProperty(
                    propertyId
                );
            if (ppDaoRes) {
                return successResponse(
...
export class FetchByUserId {
    public static async getBrandManagers(
        ...
        try {
            switch (type) {
                case 'group':
                    const daoRes =
                        await CreationDetailsByUserId.getGroupManagersGroup(
                            groupManagerId
                        );
                    if (daoRes) {
                        return successResponse(
                ...
                case 'brand':
                    const gbDaoRes =
                        await CreationDetailsByUserId.getGroupManagersBrands(
                            groupManagerId
                        );
                    if (gbDaoRes) {
                        return successResponse(
    ...
    public static async getGroupManagers(
        ...
        try {
            switch (type) {
                case 'brand':
                    const bbDaoRes =
                        await CreationDetailsByUserId.getBrandManagersBrand(
                            brandManagerId
                        );
                    if (bbDaoRes) {
                        return successResponse(
                ...
                case 'property':
                    const bpDaoRes =
                        await CreationDetailsByUserId.getBrandManagersProperty(
                            brandManagerId
                        );
                    if (bpDaoRes) {
                        return successResponse(
    ...
    public static async getPropertyManagers(
        ...
        try {
            const ppDaoRes =
                await CreationDetailsByUserId.getPropertyManagersProperty(
                    propertyManagerId
                );
            if (ppDaoRes) {
                return successResponse(
