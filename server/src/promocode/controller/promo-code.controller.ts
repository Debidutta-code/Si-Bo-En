...
export class PromoCodeController {
    ...
    public async createPromoCode(
        ...
        try {
            ...
            const result = await this.promoCodeService.createPromoCode({
                ...
                isApplicableForMobileApp:
                    promoCodeData.isApplicableForMobileApp,
                ...
                applicableRoomTypes:
                    promoCodeData.applicableRoomTypes &&
                    promoCodeData.applicableRoomTypes.length > 0
                        ? promoCodeData.applicableRoomTypes
                        : ['all'],
                applicableRatePlans:
                    promoCodeData.applicableRatePlans &&
                    promoCodeData.applicableRatePlans.length > 0
                        ? promoCodeData.applicableRatePlans
                        : ['all'],
            });
            if (result.success) {
    ...
    private validatePromoCodeData(
        ...
        if (
            ...
            promoCodeData.validFrom > promoCodeData.validTo
        ) {
            return 'Invalid validity period';
        }
        if (
            promoCodeData.minBookingAmount !== null &&
            (!promoCodeData.minBookingAmount ||
                promoCodeData.minBookingAmount < 0)
        ) {
            return 'Minimum booking amount cannot be negative';
        }
        if (
            promoCodeData.maxDiscountAmount !== null &&
            (!promoCodeData.maxDiscountAmount ||
                promoCodeData.maxDiscountAmount < 0)
        ) {
        ...
        if (
            promoCodeData.isApplicableForDesktop === false &&
            promoCodeData.isApplicableForMobileApp === false &&
            promoCodeData.isApplicableForTablet === false
        ) {
    ...
    public async getPromoCodeByParams(
        ...
        try {
            ...
            if (!propertyId || !query) {
                return res
                    .status(400)
                    .json(errorResponse('Property ID and query are required'));
            }
            const result = await this.promoCodeService.getPromoCodeByParams(
                propertyId,
                query
            );
            if (result.success) {
    ...
    public async validatePromoCodeController(
        req: PropertyRequest,
        res: Response
    ): Promise<Response> {
        try {
            ...
            if (error instanceof Error) {
                return res
                    .status(500)
                    .json(
                        errorResponse('Error occur while verifying Promocode')
                    );
            }
            return res
                .status(500)
    ...
    public async updatePromoCode(
        ...
        try {
            ...
            const { promoCodeData }: { promoCodeData: ICreatePromoCode } =
                req.body;
            if (!id) {
                return res
                    .status(400)
                    .json(errorResponse('Promo code identifier is required'));
            }
            ...
            if (
                promoCodeData.applicableRoomTypes &&
                promoCodeData.applicableRoomTypes?.length > 1
            ) {
                promoCodeData.applicableRoomTypes =
                    promoCodeData.applicableRoomTypes.filter(
                        roomType => roomType !== 'all'
                    );
            }
            if (
                promoCodeData.applicableRatePlans &&
                promoCodeData.applicableRatePlans?.length > 1
            ) {
                promoCodeData.applicableRatePlans =
                    promoCodeData.applicableRatePlans.filter(
                        ratePlan => ratePlan !== 'all'
                    );
            }
            const result = await this.promoCodeService.updatePromoCode(
                id,
                promoCodeData
            );
            if (result.success) {
        ...
        try {
            ...
            if (!propertyId || !id) {
                return res
                    ...
                    .json(
                        errorResponse(
                            'Property ID and Promo Code ID are required'
                        )
                    );
            }
            let result;
            if (isHardDelete === 'true') {
                result = await this.promoCodeService.HardDeletePromoCodeById(
                    propertyId,
                    id
                );
            } else {
                result = await this.promoCodeService.SoftDeletePromoCodeById(
                    propertyId,
                    id
                );
            }
    ...
    public async recoverPromoCode(
        ...
        try {
            ...
            const result = await this.promoCodeService.RecoverPromoCodeById(
                propertyId,
                id
            );
            if (result.success) {
