...
function RoomCard({
  ...
  // ✅ Show unavailable card instead of returning null
  if (!room.hasValidRate) {
    return (
      <motion.div
          ...
          <div className="flex flex-col sm:flex-row sm:h-[220px]">
            ...
            <div className="flex-1 flex flex-col justify-center p-4 gap-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-slate-100 text-slate-500 border border-slate-200">
                  Unavailable
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] text-slate-500 font-medium">{room.maxOccupancy} Guests max</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <Ruler size={13} className="text-slate-400 shrink-0" />
                  <span className="text-[11px] text-slate-500 font-medium">{room.roomSize} {room.roomUnit}</span>
                </div>
                {room.roomView && (
                  <div className="flex items-center gap-1.5">
                    <Eye size={13} className="text-slate-400 shrink-0" />
                    <span className="text-[11px] text-slate-500 font-medium capitalize">{room.roomView} view</span>
                  </div>
                )}
              </div>
              ...
              <p className="text-xs text-slate-400 flex items-center gap-1.5">
                ...
                No availability for the selected dates. Try adjusting your date range.
              </p>
            </div>
          </div>
        </div>
...
export default function PropertyRoomsPage() {
  ...
  // ✅ Count only rooms with valid rates
  const availableCount = roomsData.filter((r) => r.hasValidRate).length;
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            {/* ✅ Shows "0 rooms available" when all hasValidRate is false */}
