...
import { registerLoyaltyGuest } from "@/pages/rooms/services/loyalty-guest.services";
...
const buildSchema = (
  fieldConfigs: ILoyaltyProgramConfig["CreationLoyaltyConfig"]["LoyaltyProgramFieldConfig"],
) => {
  const shape: Record<string, z.ZodTypeAny> = {};

  // Always required email field
  shape["email"] = z
    .string()
  ...
  fieldConfigs
    ...
    .filter((field) => !field.fieldName.toLowerCase().includes("email"))
    .forEach((field) => {
...
export function LoyaltySignup({ className }: LoyaltySignupProps) {
  ...
  if (
    ...
    !loyaltyConfig.CreationLoyaltyConfig
  ) {
    return null;
  }

  const { CreationLoyaltyConfig } = loyaltyConfig;
  // Exclude email fields from visible fields since we have a dedicated email input
    ...
    try {
      ...
      visibleFields.forEach((field) => {
        if (data[field.fieldName]) {
          metadata[field.fieldName] = data[field.fieldName] as string;
        }
      });

      const propertyId = state.propertyDetails?.id || "";

      const result = await registerLoyaltyGuest({
        email,
        propertyId,
        metadata,
      });
      ...
      // Apply loyalty discount on success
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <p className="font-medium text-foreground">Join {programName}</p>
                ...
                <div className="space-y-2">
                  {CreationLoyaltyConfig.loyaltySpecialConditions
                    ...
                    .map((condition) => (
                      <div
                        key={condition.id}
                        className="bg-primary/5 p-2 rounded"
                      >
            ...
            <Form {...form}>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Dedicated email field — always shown */}
                  <FormField
                  ...
                  {/* Dynamic fields from config (excluding email) */}
